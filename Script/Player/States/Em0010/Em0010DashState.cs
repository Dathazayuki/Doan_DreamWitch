...
{
    public class Em0010DashState : PlayerState
    {
        ...
        public Em0010DashState(PlayerController controller) : base(controller)
        {
        ...
        public override void Enter()
        {
            ...
            controller.AnimationController?.PlayAnimation(PlayerAnimationEm0010States.DASH_START);
        }
            ...
            switch (currentPhase)
            {
                case DashPhase.Start:
                    if (phaseTimer >= START_DURATION)
                    {
                        ...
                        phaseTimer = 0f;
                        controller.AnimationController?.CrossFadeAnimation(PlayerAnimationEm0010States.DASH_LOOP, 0.05f);
                    }
                    break;
                ...
                case DashPhase.Loop:
                    if (!movement.IsDashing)
                    {
                        ...
                        phaseTimer = 0f;
                        controller.AnimationController?.CrossFadeAnimation(PlayerAnimationEm0010States.DASH_END, 0.1f);
                    }
                    break;
