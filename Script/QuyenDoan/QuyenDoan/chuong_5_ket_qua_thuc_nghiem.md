...
| **Bộ nhớ trong (RAM)** | 4 GB RAM | 8 GB RAM |
...
Để tổ chức mã nguồn rõ ràng và dễ bảo trì, kiến trúc phần mềm của trò chơi được phân rã thành các phân hệ và được trực quan hóa thông qua biểu đồ phân rã phân hệ tại Hình 5.1 và sơ đồ kiến trúc luồng dữ liệu tại Hình 5.2.
...
- **Tầng Logic & Điều phối (Controller/Logic Layer - Gói Player, Enemy, System, Trap, VFX):** Chứa toàn bộ logic xử lý nghiệp vụ của game. Trong đó, gói `Player` và `Enemy` hoạt động độc lập và chỉ giao tiếp gián tiếp qua các giao diện định nghĩa sẵn để tránh kết hợp cứng.
- **Tầng Dữ liệu & Hạ tầng (Model/Utility Layer - Gói Core, Interfaces):** Định nghĩa các hằng số, mẫu Singleton dùng chung, và đặc biệt là gói `Interfaces` chứa các giao diện trừu tượng (`IDamageable`, `IInteractable`). Gói này hoàn toàn độc lập, nằm ở đáy hệ thống và được các gói ở tầng Logic thực thi.
...
Hệ thống mã nguồn trò chơi được chia thành 8 gói chức năng chính bao gồm:
1.  **Gói Player:** Quản lý toàn bộ vòng đời, máy trạng thái, hoạt ảnh và các hành vi vật lý/chiến đấu của nhân vật chính.
2.  **Gói Enemy:** Quản lý AI quái vật tuần tra, truy đuổi dựa trên khoảng cách và các chuỗi chiêu thức tấn công của Boss.
3.  **Gói System:** Quản lý hệ thống lưu trữ/tải game cục bộ, nâng cấp làng, cửa hàng và tối ưu hóa culling thế giới.
4.  **Gói UI:** Quản lý các Canvas giao diện hiển thị tĩnh và động.
5.  **Gói Interfaces:** Định nghĩa các hợp đồng giao tiếp trừu tượng như nhận sát thương (`IDamageable`) hay tương tác (`IInteractable`).
6.  **Gói Core:** Chứa các cấu hình hằng số hệ thống và các lớp Singleton nền tảng.
7.  **Gói Trap:** Điều khiển các cạm bẫy vật lý trong môi trường.
8.  **Gói VFX:** Quản lý các hiệu ứng hạt và phản hồi hình ảnh chiến đấu.

Dưới đây là biểu đồ thiết kế gói chi tiết cho ba gói cốt lõi của hệ thống: gói Player (Hình 5.3), gói Enemy (Hình 5.4), và gói System (Hình 5.5).
...
- Lớp điều phối trung tâm `PlayerController` duy trì mối quan hệ Hợp thành (Composition) với máy trạng thái `PlayerStateMachine` (tức là máy trạng thái chỉ tồn tại khi PlayerController tồn tại và sẽ bị giải phóng cùng nhau).
...
- `PlayerFormManager` sở hữu quan hệ Kết tập (Aggregation) với các đối tượng cấu hình `PlayerFormDataSO` kế thừa từ ScriptableObject, nghĩa là các tệp dữ liệu cấu hình này tồn tại độc lập trong bộ nhớ tài sản của dự án và không bị hủy khi nhân vật bị tiêu biến.
- `PlayerController` thực thi (Implementation) giao diện trừu tượng `IDamageable` từ gói `Interfaces` để cho phép các nguồn sát thương bên ngoài gọi hàm nhận sát thương một cách đồng nhất.
...
- Lớp cơ sở trừu tượng `MvEnemyBase` kế thừa từ `MonoBehaviour` của Unity và thực thi giao diện nhận sát thương `IDamageable`. Lớp này sở hữu mối quan hệ Hợp thành (Composition) với máy trạng thái `EnemyStateMachine`.
- `EnemyStateMachine` quản lý danh sách các trạng thái `EnemyState` độc lập. Mỗi `EnemyState` duy trì một tham chiếu đến `EnemyContext` để truy xuất ngược lại dữ liệu vật lý và chỉ số của `MvEnemyBase`.
...
- Các lớp kẻ địch cụ thể như `MvEm0010` (BoneDog) sử dụng quan hệ Kế thừa (Inheritance) từ `MvEnemyBase` để tái sử dụng toàn bộ khung AI vật lý và chỉ override các cấu hình hoạt ảnh đặc thù của riêng mình.

*   **Hình 5.5: Biểu đồ thiết kế chi tiết bên trong gói System (System Package Design)**
...
*Giải thích biểu đồ gói System (Hình 5.5):*
Biểu đồ mô tả hệ thống quản lý lưu trữ và tiến trình của trò chơi:
- Lớp quản lý lưu trữ `GameSaveManager` được thiết kế theo mẫu thiết kế Singleton (lưu trữ một tham chiếu tĩnh `instance` duy nhất) để điều phối các yêu cầu lưu game trên toàn bộ các cảnh.
- Lớp này sử dụng quan hệ Phụ thuộc (Dependency) với lớp dữ liệu trung gian `GameSaveData` để thực hiện chuyển đổi dữ liệu sang chuỗi ký tự JSON.
- `GameSaveManager` kết hợp (Association) với các ScriptableObject lưu trữ trạng thái động chạy runtime như `CurrencyWalletSO` (ví tiền) và `InventoryStateSO` (túi đồ), đồng thời phụ thuộc vào lớp dịch vụ tĩnh `PortalCheckpointService` để ghi nhận các điểm dịch chuyển đã kích hoạt.

*   **Hình 5.6: Biểu đồ phân cấp kế thừa các thực thể Kẻ địch (Enemy Class Hierarchy)**
...
- **Chuỗi chiêu thức (Combo Patterns):** Thay vì ra đòn ngẫu nhiên, Boss chọn một chuỗi hành động kết hợp được lập trình sẵn (`ComboBuff` lưu mảng các trạng thái hành vi). Khi combo bắt đầu, Boss sẽ thực thi tuần tự tất cả các đòn đánh trong combo đó trước khi quay lại trạng thái đánh giá mong muốn tiếp theo. Sơ đồ luồng quyết định hành động của Boss AI được mô phỏng trong Hình 5.7.

*   **Hình 5.7: Sơ đồ luồng ra quyết định hành động của Boss AI dựa trên Desire và Combo**
...
- **Hệ thống Lưu trữ (Save/Load):** Sử dụng lớp `GameSaveManager` để serialize trạng thái trò chơi thành chuỗi dữ liệu JSON và lưu trữ lâu dài. `GameSaveManager` được thiết kế theo mẫu Singleton và nằm trên Object có thuộc tính `DontDestroyOnLoad`, đảm bảo tiến trình lưu/tải không bị gián đoạn khi chuyển cảnh.
- **Hệ thống Culling tối ưu hóa hiệu năng (Culling System):** Bản đồ được chia thành các khu vực phòng độc lập (`RoomController`). Khi người chơi di chuyển ra khỏi phạm vi hoạt động của một phòng chơi, `CullingManager` sẽ tự động vô hiệu hóa (`SetActive(false)`) toàn bộ Sprite Renderers, Collision Colliders và tắt các script tính toán AI của quái vật thuộc phòng đó. Các đối tượng sẽ được kích hoạt lại ngay lập tức khi người chơi bước vào vùng biên giới tiếp giáp phòng, giúp giảm thiểu đáng kể số lượng lệnh vẽ (Draw Calls) và các phép tính toán vật lý không cần thiết (Hình 5.8).
...
*   **Hình 5.8: Hình vẽ trực quan các phòng chơi bị tắt hiển thị ngoài tầm nhìn để tối ưu hóa**
    ![culling_vis](file:///d:/du%20an/Doan_DreamKnight/Assets/Project/Script/QuyenDoan/QuyenDoan/Hinhve/CullingVisualization.png)

#### d. Kiến trúc giao diện người dùng và Canvas (UI Canvas Architecture)
...
*   **Hình 5.9: Biểu đồ lớp chi tiết của lớp PlayerController**
...
#### a. Đặc tả chi tiết lớp PlayerController (Hình 5.9)
...
#### b. Đặc tả chi tiết lớp MvEnemyBase (Hình 5.10)
...
#### c. Đặc tả chi tiết lớp GameSaveManager (Hình 5.11)
...
1.  **Use Case Chuyển đổi linh hồn (Form Switch):** Sơ đồ trình tự tương tác giữa `PlayerInput`, `PlayerController`, `PlayerFormManager`, và `PlayerStats` khi thực hiện thay đổi hình dạng nhân vật được biểu diễn chi tiết tại Hình 5.12.
2.  **Use Case Lưu trữ tiến trình game (Save Game):** Khi người chơi tương tác cầu nguyện tại đền thờ hồi sinh (`RespawnShrine`), một yêu cầu lưu game được gửi tới `GameSaveManager`. Bộ quản lý sẽ gọi hàm `CaptureSaveData()` để thu thập dữ liệu từ các ScriptableObjects trạng thái (`CurrencyWalletSO`, `InventoryStateSO`, v.v.) và các nâng cấp từ `FacilityManager`, biên dịch thành cấu trúc trung gian `GameSaveData`. Đối tượng này được chuyển đổi thành chuỗi JSON qua bộ tiện ích của Unity và ghi xuống đĩa cứng dưới dạng tệp tin văn bản cục bộ.

*   **Hình 5.12: Sơ đồ trình tự quá trình đồng bộ hóa chuyển đổi hình thái (Form Switch)**
...
Hệ thống quản lý lưu trữ được phân tách thành hai dạng độc lập: lưu trữ cấu hình hệ thống bằng cơ chế `PlayerPrefs` và lưu trữ tiến trình chơi game bằng định dạng JSON. Các thuộc tính và thành phần dữ liệu được quy định chi tiết bằng cấu trúc danh sách đặc tả dưới đây:

**Lưu trữ cấu hình hệ thống (qua PlayerPrefs):**
Các thiết lập hệ thống cơ bản được ghi nhận cục bộ dưới dạng cặp khóa - giá trị bao gồm:
...
Khi người chơi kích hoạt tệp `DreamWitch.exe`, hệ thống hiển thị màn hình bắt đầu với âm nhạc u sầu đặc trưng (Hình 5.13). Giao diện cung cấp các tùy chọn để bắt đầu chơi mới hoặc tiếp tục tiến trình cũ. Khi chọn "Continue", giao diện Save Slot hiển thị thông tin chi tiết về từng ô lưu trữ (thời gian chơi, lượng vàng tích lũy, các chỉ số cơ bản) giúp người chơi quản lý tiến trình (Hình 5.14).

*   **Hình 5.13: Màn hình khởi động trò chơi (Title Screen)**
    ![TitleScreen](file:///d:/du%20an/Doan_DreamKnight/Assets/Project/Script/QuyenDoan/QuyenDoan/Hinhve/TitleScreen.jpg)
*   **Hình 5.14: Giao diện chọn Slot tiếp tục chơi (Save Slot Menu)**
