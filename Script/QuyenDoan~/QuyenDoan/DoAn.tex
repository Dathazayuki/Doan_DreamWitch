...
\usepackage{tabularx}
\usepackage{float} % Bắt buộc phải có gói này để dùng thuộc tính cố định vị trí [H] cho bảng
\usepackage{listings}
\usepackage{float}
...
\usepackage[nonumberlist, nopostdot, nogroupskip, toc]{glossaries}
...
\def \TITLE{Phát triển game hành động phiêu lưu 2D ứng dụng kỹ thuật Distance Culling (Dream Witch)}
\def \AUTHOR{TRỊNH MINH ĐẠT}
...
\glsaddall 
 \renewcommand*{\glossaryname}{DANH MỤC THUẬT NGỮ VÀ TỪ VIẾT TẮT}
...
\chapter{NGƯỜI CHƠI VÀ ĐỘNG LỰC NGƯỜI CHƠI}
...
\newpage
%\pagestyle{fancy} % Áp dụng header và footer
\chapter{THIẾT KẾ TRÒ CHƠI}
\label{chapter:GameDesign}
\subfile{Chuong/3_Thiet_ke_tro_choi}

\newpage
% \newpage
% \chapter{THIẾT KẾ TRIỂN KHAI HỆ THỐNG}
% \label{chapter:SystemDesignImplementation}
% \subfile{Chuong/4_Thiet_ke_trien_khai_he_thong}
